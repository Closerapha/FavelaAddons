   ...
   public static String describe() {
      ...
      if (client.player != null && client.level != null) {
         ...
         for(Entity entity : client.level.getEntities(client.player, new AABB(eye, end).inflate(RADIUS))) {
            Vec3 target = FavelaDisplays.renderedPosition(entity);
         ...
         hits.sort(Comparator.comparingDouble((entity) -> eye.distanceTo(FavelaDisplays.renderedPosition(entity))));
         ...
         for(Entity entity : hits) {
            ...
            out.append(String.format(Locale.ROOT, "%n  §f%.1fm §7%s%s", distance, flag, FavelaDebugger.identityOf(entity, String.valueOf(EntityType.getKey(entity.getType())))));
            String gear = FavelaGear.describe(entity);
            if (!gear.isEmpty()) {
