...
public class FavelaModMenu implements ModMenuApi {
   ...
   public ConfigScreenFactory<?> getModConfigScreenFactory() {
      return (parent) -> {
         ...
         secVuln.add(entryBuilder.startIntField(Component.literal("Check Every (ticks)"), Config.vulnCheckTicks).setDefaultValue(2).setMin(1).setMax(20).setTooltip(new Component[]{Component.literal("20 ticks = 1 second.")}).setSaveConsumer((newValue) -> Config.vulnCheckTicks = newValue).build());
         secVuln.add(entryBuilder.startStrField(Component.literal("Bar Texture"), Config.vulnFillTexture).setDefaultValue("xikage/default/inner").setTooltip(new Component[]{Component.literal("Which piece of the floating bar carries the colour."), Component.literal("Only touch this if the server changes its HUD.")}).setSaveConsumer((newValue) -> Config.vulnFillTexture = newValue).build());
