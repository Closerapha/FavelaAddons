...
public class FavelaPortals {
   private static final java.util.Set<Integer> seen = new java.util.HashSet();
   private static int cooldown = 0;

   public static void registrar() {
      ClientTickEvents.END_CLIENT_TICK.register(FavelaPortals::onTick);
   }
      ...
      if (!Config.splits || client.level == null || client.player == null) {
         ...
         for(Entity entity : client.level.getEntities(client.player, client.player.getBoundingBox().inflate(range))) {
            if (!((double)entity.distanceTo(client.player) > range)) {
               ...
               if (!gear.isEmpty() && FavelaDisplays.matchesAny(gear, Config.parsedPortalModels) && seen.add(entity.getId())) {
                  FavelaSplits.onPortal(gear, secondsAbove(client, entity), String.format(java.util.Locale.ROOT, "a %.0fm em %.0f %.0f %.0f", entity.distanceTo(client.player), entity.getX(), entity.getY(), entity.getZ()));
               }
            }
         }
