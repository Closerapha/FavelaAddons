...
public class SapoAutoWalls {
   ...
   public static void registrar() {
      ClientTickEvents.END_CLIENT_TICK.register(SapoAutoWalls::onTick);
      LevelRenderEvents.BEFORE_GIZMOS.register(SapoAutoWalls::render);
   }
   ...
   private static void onTick(Minecraft client) {
      ...
      if (Config.autoWalls && client.level != null && client.player != null ) {
         try {
            ...
            gapTarget = null;
            if (Config.devMode) {
               System.out.println("[Sapo Auto Walls] Error scanning ring: " + e.getMessage());
            }
         }
      ...
      if (gapTarget == null) {
         ...
         turnArmed = true;
         if (Config.devMode) {
            System.out.println("[Sapo Auto Walls] New gap detected, aiming once.");
         }
      }
   ...
   private static void turnToGap(Minecraft client) {
      ...
      if (active && target != null && client.player != null) {
         ...
         if (Math.abs(deltaYaw) <= step) {
            ...
            turnArmed = false;
            if (Config.devMode) {
               System.out.println("[Sapo Auto Walls] Aimed at the gap.");
            }
         } else {
   ...
   public static String diagnose(Minecraft client) {
      ...
      if (client.level == null || client.player == null) {
         ...
         for(Entity entity : client.level.getEntities(client.player, box)) {
            ...
            if (modelId != null) {
               ++displays;
               if (SapoDisplays.matchesAny(modelId, Config.parsedAutoWallsModels)) {
                  ++matched;
               }
            }
         }
   ...
   private static Vec3 findGap(Minecraft client, StringBuilder diag) {
      ...
      for(Entity entity : client.level.getEntities(client.player, box)) {
         ...
         if (modelId != null) {
            ...
            if (guarded && distance <= bossRange && SapoDisplays.matchesAny(modelId, Config.parsedAutoWallsBossModels)) {
               bossFound = true;
               if (bossName == null) {
            ...
            if (distance <= wallRange && SapoDisplays.matchesAny(modelId, Config.parsedAutoWallsModels)) {
   ...
   private static void render(LevelRenderContext context) {
      ...
      if (Config.autoWalls && Config.autoWallsTracer && target != null && client.player != null) {
         try {
            ...
            if (Config.devMode) {
               System.out.println("[Sapo Auto Walls] Error drawing tracer: " + e.getMessage());
            }
         }
