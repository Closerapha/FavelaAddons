...
import com.sapo.client.mixin.SapoBossBarAccessor;

public class SapoCalls {
   ...
   public static void registrar() {
      ClientTickEvents.END_CLIENT_TICK.register(SapoCalls::onTick);
      HudElementRegistry.addLast(Identifier.parse("sapo:calls"), SapoCalls::render);
   }

   public static Collection<LerpingBossEvent> bossBars() {
      try {
         Gui gui = Minecraft.getInstance().gui;
         return (Collection<LerpingBossEvent>)(gui == null ? List.of() : ((SapoBossBarAccessor)gui.getBossOverlay()).sapo$events().values());
      } catch (Throwable e) {
         if (Config.devMode) {
