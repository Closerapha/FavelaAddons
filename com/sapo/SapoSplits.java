...
public class SapoSplits {
   ...
   public static void registrar() {
      loadTable();
      ClientTickEvents.END_CLIENT_TICK.register(SapoSplits::onTick);
      HudElementRegistry.addLast(Identifier.parse("sapo:splits"), SapoSplits::render);
   }
   ...
   private static void onTick(Minecraft client) {
      ...
      LerpingBossEvent bar = SapoBossHp.activeBar();
      String name = bar == null ? "" : SapoAutoClicker.sanitize(bar.getName().getString());
      ...
      if (Config.splits) {
         ...
         if (unmatched.add(tail)) {
            ...
            if (client.player != null) {
               client.player.sendSystemMessage(Component.literal("§e[FavelaClient] Portal sem rota: §f" + tail));
            }
   ...
   public static void logChat(String text) {
      if (Config.devMode) {
         ...
         if (!plain.isEmpty()) {
            try {
               File folder = new File(FabricLoader.getInstance().getConfigDir().toFile(), "sapo/debug");
               if (!folder.exists()) {
                  folder.mkdirs();
               }
   ...
   private static void loadBests() {
      ...
      try {
         File file = new File(new File(FabricLoader.getInstance().getConfigDir().toFile(), "sapo"), "pb.json");
         if (file.exists()) {
            InputStreamReader reader = new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8);
